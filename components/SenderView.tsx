...
import { Upload, Folder, File as FileIcon, CheckCircle, Copy, Check, Loader2, FilePlus, Globe } from 'lucide-react';
import { transferService } from '../services/webRTCService';
import { createManifest, formatBytes } from '../utils/fileUtils';
import { motion } from 'framer-motion';
import { SenderStatus } from '../types';
...
const SenderView: React.FC<SenderViewProps> = ({ onComplete }) => {
  ...
  const processFiles = async (fileList: FileList) => {
    ...
    // 모든 파일 전송을 P2P로 처리
    startP2P(manifest, files);
  };

  const startP2P = async (manifest: any, files: File[]) => {
    setStatus('WAITING');
    ...
    <div className="flex flex-col items-center justify-center h-full w-full max-w-2xl mx-auto p-6 z-10 relative">
      ...
      {status === 'IDLE' && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full space-y-4">
           <div onDrop={handleDrop} onDragOver={handleDragOver} className="border-2 border-dashed border-cyan-500/50 bg-black/40 backdrop-blur-md rounded-3xl p-10 text-center transition-all flex flex-col items-center justify-center min-h-[320px]">
             <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileSelect} multiple />
             <input type="file" className="hidden" ref={folderInputRef} onChange={handleFileSelect} multiple
               // @ts-ignore
               webkitdirectory=""
             />
             <div className="mb-8">
             ...
             <div className="flex gap-4 w-full max-w-md">
               <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white py-4 rounded-xl flex flex-col items-center gap-2 transition-all group">
               ...
               <button onClick={() => folderInputRef.current?.click()} className="flex-1 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white py-4 rounded-xl flex flex-col items-center gap-2 transition-all group">
      ...
      {status === 'WAITING' && roomId && shareLink && (
        <motion.div className="bg-black/60 backdrop-blur-xl p-8 rounded-3xl border border-cyan-500/30 flex flex-col items-center max-w-md w-full">
          ...
          <div className="w-full bg-gray-900/50 p-4 rounded-lg mb-4 text-left border border-gray-700">
             ...
             <p className="text-xs text-gray-400 pl-9">{manifest?.totalFiles} files • {formatBytes(manifest?.totalSize || 0)}</p>
          </div>
          <div className="flex gap-2 w-full">
            <div className="flex-1 bg-gray-800 rounded px-3 py-2 text-xs text-gray-400 truncate leading-8 font-mono">{shareLink}</div>
            <button onClick={copyToClipboard} className="bg-cyan-600 hover:bg-cyan-500 text-white p-2 rounded transition-colors">{copied ? <Check size={16}/> : <Copy size={16}/>}</button>
          </div>
          <p className="mt-6 text-sm text-gray-500 flex items-center gap-2"><Loader2 className="animate-spin w-4 h-4" /> Waiting for receiver...</p>
        </motion.div>
      )}
      ...
      {(status === 'TRANSFERRING' || status === 'CONNECTING') && (
        <div className="w-full space-y-6 max-w-lg">
          ...
          <div className="relative h-4 bg-gray-800 rounded-full overflow-hidden border border-gray-700">
            <motion.div className="absolute top-0 left-0 h-full bg-gradient-to-r from-cyan-500 to-purple-600" initial={{ width: 0 }} animate={{ width: `${progressData.progress}%` }} />
          </div>
     ...
     {status === 'DONE' && (
        <div className="text-center">
          ...
          <button onClick={() => window.location.reload()} className="bg-white text-black px-8 py-3 rounded-full font-bold hover:bg-cyan-50 transition-colors">Send Another</button>
        </div>
      )}
    </div>
  );
