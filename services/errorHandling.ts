...
class ErrorHandler {
  ...
  private initializeRetryConfigs() {
    ...
    this.retryConfigs.set(ErrorType.TURN_CONNECTION_FAILED, {
      ...
      maxDelay: 10000,
      backoffMultiplier: 2,
      retryableErrors: [ErrorType.TURN_CONNECTION_FAILED, ErrorType.TURN_SERVER_UNAVAILABLE]
    });
  ...
  public classifyError(error: any, context?: Record<string, any>): ErrorInfo {
    ...
    if (error.message) {
      ...
      if (message.includes('turn') || message.includes('relay')) {
        if (message.includes('credential') || message.includes('unauthorized') || message.includes('401')) {
          errorType = ErrorType.TURN_CREDENTIALS_EXPIRED;
          severity = ErrorSeverity.HIGH;
        } else if (message.includes('unavailable') || message.includes('timeout') || message.includes('connection')) {
        ...
        severity = ErrorSeverity.MEDIUM;
      } else if (message.includes('peer') || message.includes('webrtc') || message.includes('ice failed')) {
        errorType = ErrorType.PEER_CONNECTION_FAILED;
        severity = ErrorSeverity.HIGH;
      } else if (message.includes('signaling') || message.includes('socket') || message.includes('connection')) {
        errorType = ErrorType.SIGNALLING_CONNECTION_FAILED;
        severity = ErrorSeverity.HIGH;
      } else if (message.includes('network') || message.includes('offline') || message.includes('dns')) {
        errorType = ErrorType.NETWORK_UNAVAILABLE;
        severity = ErrorSeverity.CRITICAL;
      } else if (message.includes('file') || message.includes('transfer') || message.includes('chunk')) {
  ...
  public canRetry(errorInfo: ErrorInfo): boolean {
    ...
    return errorInfo.retryCount < config.maxRetries &&
  ...
  public calculateRetryDelay(errorInfo: ErrorInfo): number {
    ...
    const delay = config.baseDelay * Math.pow(config.backoffMultiplier, errorInfo.retryCount);
  ...
  public async retryWithError<T>(
    ...
    console.log(`[ErrorHandler] Retrying ${errorInfo.type} (attempt ${errorInfo.retryCount}/${errorInfo.maxRetries}) after ${delay}ms`);
  ...
  public async executeWithRetry<T>(
    ...
    try {
      ...
      if (errorInfo.type !== errorType) {
        console.warn(`[ErrorHandler] Expected ${errorType} but got ${errorInfo.type}`);
      }
  ...
  // 에러 콜백 트리거
  private triggerErrorCallbacks(errorType: ErrorType, errorInfo: ErrorInfo): void {
    const callbacks = this.errorCallbacks.get(errorType);
    if (callbacks) {
  ...
  // 에러 기록 정리
  public clearErrors(olderThanMs: number = 3600000): void { // 기본 1시간
    const cutoffTime = Date.now() - olderThanMs;
    this.errors = this.errors.filter(error => error.timestamp > cutoffTime);
    console.log(`[ErrorHandler] Cleared ${this.errors.length} old error records`);
  }
  ...
  public async checkNetworkConnectivity(): Promise<{
    ...
    try {
      ...
      const signalingTimeout = setTimeout(() => signalingController.abort(), 3000);
