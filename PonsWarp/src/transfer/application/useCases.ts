...
export interface TransferRunner {
  ...
  cancel(error?: string): Promise<void>;
...
export function createTransferRunner(
  options: TransferRunnerOptions
): TransferRunner {
  if (!finite(options.totalBytes) || !finite(options.timeoutMs))
    throw new TypeError('Numeric options must be finite');
  if (options.role === 'receiver' && !options.ports.writer)
    throw new TypeError('A writer is required for receiver execution');
  let current = createTransferState({
    role: options.role,
  ...
  const dispatch = (event: TransferEvent) => {
    ...
    current = reduceTransferState(current, event);
    if (current === before) return;
    events.push(event);
    output?.emit(event);
    output?.setState?.(current);
  };
  const clearTimer = () => {
    if (timer !== undefined) {
  ...
  const armTimeout = () => {
    ...
    if (options.timeoutMs && options.timeoutMs > 0)
      timer = clock.setTimeout(() => {
        void terminate('timeout', 'Transfer timed out');
      }, options.timeoutMs);
  };
  const bindPeer = (p: PeerPort) => {
    ...
    unsubscribe = p.onMessage(message => {
      ...
      if (m.type === 'chunk' && options.role === 'receiver') {
        ...
        const id =
          typeof m.chunkId === 'string' || typeof m.chunkId === 'number'
            ? m.chunkId
            : undefined;
        if (
          id !== undefined &&
          (current.seenChunks.has(id) || pendingWrites.has(id))
        ) {
          dispatch({
        ...
        const write = async () => {
          if (terminated) return;
          await options.ports.writer!.write(
            m.data,
            typeof m.offset === 'number' && Number.isFinite(m.offset)
              ? m.offset
              : undefined
          );
          if (!terminated)
            await runner.progress(
              m.bytes as number,
              typeof m.totalBytes === 'number' && Number.isFinite(m.totalBytes)
                ? m.totalBytes
                : undefined,
              id
            );
        ...
        void runner.progress(
          ...
          typeof m.totalBytes === 'number' && Number.isFinite(m.totalBytes)
            ? m.totalBytes
            : undefined,
          m.chunkId as string | number | undefined
        );
        ...
        m.type === 'resume' &&
        (m.bytes === undefined ||
          (typeof m.bytes === 'number' && Number.isFinite(m.bytes)))
      )
        runner.resume(m.bytes as number | undefined);
    });
    if (p.onClose)
      unsubscribeClose = p.onClose(() => {
        if (!closed) {
          connected = false;
          clearTimer();
          dispatch({ type: 'reconnect', role: options.role, at: clock.now() });
        }
      });
  };
  const establish = async (kind: 'connect' | 'join', room?: string) => {
    ...
    try {
      ...
      if (kind === 'connect') await options.ports.signaling.connect();
      else await options.ports.signaling.join(room!);
      ...
      if (options.role === 'sender')
        dispatch({ type: 'ready', role: options.role, at: clock.now() });
      armTimeout();
    } catch (error) {
      connected = false;
      clearTimer();
  ...
  const runner: TransferRunner = {
    ...
    connect: () => establish('connect'),
    join: room => establish('join', room),
    sendChunk: async (data, bytes, chunkId, offset) => {
      if (!Number.isFinite(bytes) || !finite(offset))
      ...
      await runner.progress(bytes, current.totalBytes, chunkId);
    },
    resume: bytes => {
      if (finite(bytes))
        dispatch({
    ...
    reconnect: async () => {
      ...
      dispatch({ type: 'reconnect', role: options.role, at: clock.now() });
      await establish('connect');
    },
    progress: async (bytes, totalBytes = current.totalBytes, chunkId) => {
      if (!Number.isFinite(bytes) || !Number.isFinite(totalBytes))
        throw new TypeError('Numeric values must be finite');
      if (chunkId !== undefined && current.seenChunks.has(chunkId)) {
        dispatch({
          type: 'duplicate-chunk',
          role: options.role,
          chunkId,
    ...
    cancel: async error => {
      await terminate('cancel', error);
    },
    timeout: error => {
      void terminate('timeout', error);
    },
    error: message => {
      void terminate('error', message);
    },
    close: async () => {
      if (closed) return;
      closed = true;
...
export const createSenderUseCase = (
  ports: TransferPorts,
  totalBytes = 0,
  timeoutMs?: number
) => createTransferRunner({ role: 'sender', ports, totalBytes, timeoutMs });
export type ReceiverTransferPorts = Omit<TransferPorts, 'writer'> & {
  writer: WriterPort;
};
export const createReceiverUseCase = (
  ports: ReceiverTransferPorts,
  totalBytes = 0,
  timeoutMs?: number
) => createTransferRunner({ role: 'receiver', ports, totalBytes, timeoutMs });
export interface CloudDropUseCase {
