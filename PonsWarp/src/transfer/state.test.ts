...
const apply = (role: 'sender' | 'receiver', events: TransferEvent[]) =>
  events.reduce(
    reduceTransferState,
    createTransferState({ role, totalBytes: 100 })
  );
...
describe('transfer core reducer', () => {
  ...
  it('ignores duplicate chunks and all late terminal/error events', () => {
    ...
    const done = reduceTransferState(duplicate, {
      type: 'complete',
      role: 'receiver',
    });
    expect(
      reduceTransferState(done, {
        type: 'error',
        role: 'receiver',
        error: 'late',
      })
    ...
    expect(
      reduceTransferState(done, { type: 'cancel', role: 'receiver' })
    ).toBe(done);
  });
  ...
  it('preserves progress through reconnect and resumes deterministically', () => {
    ...
    state = reduceTransferState(state, {
      type: 'progress',
      role: 'sender',
      bytes: 30,
      totalBytes: 100,
