...
function assertEntryFields(
  ...
  if (!isRecord(value) || fields.some(field => !isString(value[field]))) {
    throw new Error(
      `Compatibility manifest ${section} entry ${index} is malformed`
    );
  }
}
...
function assertUnique(values: string[], section: string): void {
  if (new Set(values).size !== values.length) {
    throw new Error(
      `Compatibility manifest contains duplicate ${section} names`
    );
  }
}

export function assertCompatibilityManifest(
  value: unknown
): asserts value is CompatibilityManifest {
  if (!isRecord(value))
    throw new Error('Compatibility manifest must be an object');
  for (const key of ['contract', 'version', 'status']) {
    if (!isString(value[key]))
      throw new Error(`Compatibility manifest requires string field: ${key}`);
  }
  for (const key of arraySections) {
    if (!Array.isArray(value[key]))
      throw new Error(`Compatibility manifest requires array section: ${key}`);
  }
  for (const key of objectSections) {
    if (!isRecord(value[key]))
      throw new Error(`Compatibility manifest requires object section: ${key}`);
  }

  (value.constructors as unknown[]).forEach((entry, index) =>
    assertEntryFields(entry, 'constructor', index, ['module', 'export', 'kind'])
  );
  (value.singletonExports as unknown[]).forEach((entry, index) =>
    assertEntryFields(entry, 'singleton export', index, [
      'module',
      'name',
      'type',
    ])
  );
  (value.namedExports as unknown[]).forEach((entry, index) =>
    assertEntryFields(entry, 'named export', index, ['module', 'name', 'type'])
  );
  (value.callsites as unknown[]).forEach((entry, index) =>
    assertEntryFields(entry, 'callsite', index, ['module', 'uses'])
  );

  const methods = value.methods as unknown[];
  methods.forEach((method, index) =>
    assertEntryFields(method, 'method', index, ['owner', 'name', 'signature'])
  );
  assertUnique(
    methods.map(
      method =>
        `${(method as Record<string, string>).owner}.${(method as Record<string, string>).name}`
    ),
    'API'
  );
  ...
  for (const [owner, names] of Object.entries(events)) {
    if (
      !isString(owner) ||
      !Array.isArray(names) ||
      names.some(name => !isString(name))
    ) {
      throw new Error(
        `Compatibility manifest events for ${owner} are malformed`
      );
    }
  ...
  if (
    Object.entries(roles).some(
      ([role, description]) => !isString(role) || !isString(description)
    )
  ) {
...
export const compatibilityManifest: CompatibilityManifest =
  cloneAndFreeze(manifestJson);

export function loadCompatibilityManifest(): CompatibilityManifest {
  return cloneAndFreeze(
    JSON.parse(JSON.stringify(compatibilityManifest)) as CompatibilityManifest
  );
}
