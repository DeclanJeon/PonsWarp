...
- **Integrated Rust signaling**: the canonical in-repo `../ponswarp-signaling-rs/` backend for WebSocket signaling, health/readiness checks, Cloud Drop APIs, and optional TURN credentials.
...
flowchart LR
  ...
  frontendA <-->|room code, offer, answer, ICE| signaling[Rust ../ponswarp-signaling-rs WebSocket /ws]
...
| Rust signaling and Cloud Drop backend | `../ponswarp-signaling-rs/` |
...
Backend production env should be configured in `../ponswarp-signaling-rs/.env.production` or a secret manager/env file read by the process, not in frontend `VITE_*` values.
...
3. Run focused checks for the area you changed, such as `pnpm type-check`, `pnpm test`, `pnpm build`, or `cargo build` in `../ponswarp-signaling-rs`.
