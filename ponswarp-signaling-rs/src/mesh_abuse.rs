...
impl RateLimiter for InMemoryTokenBucketRateLimiter {
    fn check<'a>(&'a self, key: &'a str, now_seconds: u64) -> BoxFuture<'a, RateLimitOutcome> {
        async move {
            ...
            let reset_seconds = now_seconds
                + seconds_until_full(bucket.tokens, self.capacity, self.refill_per_second);
            if bucket.tokens == 0 {
...
const POSTGRES_TOKEN_BUCKET_CLEANUP_SQL: &str =
    "DELETE FROM mesh_rate_limits WHERE expires_at <= $1";
    ...
    fn check<'a>(&'a self, key: &'a str, now_seconds: u64) -> BoxFuture<'a, RateLimitOutcome> {
        async move {
            ...
            if self.cleanup_expired
                && sqlx::query(POSTGRES_TOKEN_BUCKET_CLEANUP_SQL)
                    .bind(now)
                    .execute(&self.pool)
                    .await
                    .is_err()
            ...
            let row = sqlx::query(POSTGRES_TOKEN_BUCKET_UPSERT_SQL)
                .bind(key)
                .bind(capacity)
                ...
                Ok(None) => {
                    return RateLimitOutcome::Limited {
                        ...
                        reset_seconds: now_seconds
                            + seconds_until_full(0, self.capacity, self.refill_per_second),
                    };
                }
                Err(_) => {
                    return RateLimitOutcome::StorageUnavailable {
            ...
            let remaining_tokens = row
                .try_get::<f64, _>("tokens")
                .unwrap_or(0.0)
                .floor()
                .max(0.0);
...
impl MeshMetrics {
    ...
    pub fn record_rate_limit_storage_failure(&self) {
        self.rate_limit_storage_failures
