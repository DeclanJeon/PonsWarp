...
use crate::mesh_repository::{repository_from_config, MeshRepository, MeshState};
...
impl AppState {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let (mesh_repository, mesh_rate_limiter) = if config.mesh.enabled
            && config.mesh.storage == MeshStorage::Postgres
        {
            let pool = PgPoolOptions::new()
                ...
                .await?;
            let repository =
                crate::mesh_repository::PostgresMeshRepository::from_pool(&config, pool.clone())
                    .await?
                    .expect("postgres repository required by configuration");
            let limiter = Arc::new(PostgresTokenBucketRateLimiter::new(
                pool,
                config.mesh.rate_limit_capacity,
                config.mesh.rate_limit_refill_per_second,
            ));
            (
                Arc::new(repository) as Arc<dyn MeshRepository>,
                limiter as Arc<dyn RateLimiter>,
            )
        } else {
            (
                repository_from_config(&config, mesh.clone()).await?,
