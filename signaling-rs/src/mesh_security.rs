...
pub fn can_workspace(
    ...
    match (role, action) {
        ...
        (Some(WorkspaceRole::Reader), WorkspaceAction::ReadMetadata) => true,
        _ => false,
    }
}
...
pub fn verify_connect_grant(
    grant: &str,
    expected_share_code: &str,
    expected_file_id: &str,
    expected_source_node_id: &str,
    expected_receiver_fingerprint: &str,
    now: u64,
    pepper: &str,
    ...
    let expected_sig = hmac_sha256_bytes(pepper.as_bytes(), &payload);
    ...
    if claims.share_hash != hash_secret(expected_share_code, pepper, "share-code")?.id
        || claims.file_id != expected_file_id
        || claims.source_node_id != expected_source_node_id
        || claims.receiver_fingerprint != expected_receiver_fingerprint
    {
